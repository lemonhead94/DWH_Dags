# Description: This script cleans the data from the hls csv file
...
def __clean_birth_date(df: pd.DataFrame) -> None:
    ...
    df.fillna({"cleaned_birth_date_formatted": df.hhb_birth_date}, inplace=True)
...
def __clean_death_date(df: pd.DataFrame) -> None:
    ...
    df.fillna({"cleaned_death_date_formatted": df.hhb_death_date}, inplace=True)
...
def main() -> None:
    ...
    __replace_empty_strings_and_zero_width_spaces(df_updates)
    __replace_empty_strings_and_zero_width_spaces(df_new)

    __clean_birth_date(df_updates)
    __clean_birth_date(df_new)

    __clean_death_date(df_updates)
    __clean_death_date(df_new)
